...
Install components from this registry with `npx shadcn@latest add` and the JSON URL from each docs page. Components work with provider-optional hooks and sensible defaults. Add `ThemeProvider`, `MotionProvider`, or `UnicodeProvider` only when the application needs an override; provider files install into `providers/`.

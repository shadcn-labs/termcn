...
describe("search command", () => {
  ...
  it("prints human-readable output by default", async () => {
    ...
    await expect(
      search.parseAsync(["@termcn", "--cwd", "/tmp/test-project"], {
        from: "user",
      })
    ).rejects.toThrow("process.exit:0");
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Found 2 items in @termcn")
    );
    expect(log).toHaveBeenCalledWith(expect.stringContaining("@termcn/button"));
  ...
  it("prints JSON output with --json", async () => {
    ...
    await expect(
      search.parseAsync(["@termcn", "--cwd", "/tmp/test-project", "--json"], {
        from: "user",
      })
    ).rejects.toThrow("process.exit:0");
  ...
  it("errors on an unknown --type", async () => {
    ...
    await expect(
      search.parseAsync(
        ["@termcn", "--type", "bogus", "--cwd", "/tmp/test-project"],
        {
          from: "user",
        }
      )
  ...
  it("searches all configured registries when none are provided", async () => {
    ...
    // Only the configured registries are searched (builtin @termcn is

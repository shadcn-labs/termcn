...
const TERMCN_URL = REGISTRY_URL.replace(/\/r\/?$/, "");
...
describe("resolveApplyInitUrl", () => {
  ...
  it("should include the inferred template for preset codes", () => {
    ...
    expect(parsed.origin + parsed.pathname).toBe(`${TERMCN_URL}/init`);
  ...
  it("should keep the current base for raw preset URLs without injecting a template", () => {
    const presetUrl = `${TERMCN_URL}/init?base=radix&style=nova&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=inter&rtl=false&menuAccent=subtle&menuColor=default&radius=default`;
  ...
  it("should preserve only from raw preset URLs", () => {
    const presetUrl = `${TERMCN_URL}/init?base=radix&style=nova&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=inter&rtl=false&menuAccent=subtle&menuColor=default&radius=default&only=font`;
    const initUrl = resolveApplyInitUrl(presetUrl, "base", {
      template: "next",
      rtl: true,
    });
  ...
  it("rejects missing only values with allowed values", () => {
    expect(() => resolveApplyOnly(true)).toThrow(
      [
        ...
        "Example: termcn apply <preset> --only theme,font.",
      ].join("\n")
    );
  });
...
describe("getPresetUrlOnly", () => {
  it("reads only from termcn init URLs", () => {
    const presetUrl = `${TERMCN_URL}/init?base=radix&style=nova&only=font`;

    expect(getPresetUrlOnly(presetUrl)).toBe("font");
  });

  it("reads only from non-termcn init URLs", () => {
    const presetUrl =
  ...
  it("rejects only without a preset", () => {
    expect(() => validateApplyOnlyPreset({ only: ["theme"] })).toThrow(
      [
        ...
        "Use: termcn apply <preset> --only theme,font.",
      ].join("\n")
    );
  });

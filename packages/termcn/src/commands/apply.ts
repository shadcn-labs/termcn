      ...
      if (!preset) {
        ...
        await promptToOpenPresetBuilder({
          ...
          followUp: `Then run ${highlighter.info(
            "termcn apply --preset <preset>"
          )} with the preset code or preset URL from termcn.dev.`,
          prompt: !options.yes,
        });
  ...
  if (!knownPresetNames.includes(preset)) {
    logger.error(
      `Invalid preset: ${highlighter.info(
        preset
      )}.\nUse one of the available presets: ${knownPresetNames.join(", ")} \nor build your own at ${highlighter.info(`${TERMCN_URL}/create`)}`
    );
    logger.break();
    process.exit(1);
  }
...
export function resolveApplyInitUrl(
  ...
  if (isUrl(preset)) {
    ...
    if (url.pathname === "/init" && preset.startsWith(TERMCN_URL)) {
  ...
  const resolvedPreset =
    DEFAULT_PRESETS[preset as keyof typeof DEFAULT_PRESETS];
...
function getInitCommand(preset?: string) {
  ...
  return `termcn init --preset ${quoteShellArg(preset)}`;
}

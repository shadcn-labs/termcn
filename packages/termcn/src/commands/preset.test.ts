...
describe("preset commands", () => {
  ...
  it("prints decoded preset JSON with derived values", async () => {
    ...
    await presetCommand.parseAsync(["decode", "a0", "--json"], {
      from: "user",
    });
  ...
  it("rejects URLs", () => {
    expect(() =>
      decodePresetCode("https://termcn.dev/create?preset=a0")
    ).toThrow("Invalid preset code: https://termcn.dev/create?preset=a0");
  });
  ...
  it("does not open invalid preset codes", async () => {
    ...
    await expect(
      openCommand.parseAsync(["https://termcn.dev/create?preset=a0"], {
        from: "user",
      })
    ).rejects.toThrow("process.exit:1");

    expect(logger.error).toHaveBeenCalledWith(
      "Invalid preset code: https://termcn.dev/create?preset=a0"
    );
  ...
  it("resolves a project preset using the existing resolver", async () => {
    ...
    expect(logger.log).toHaveBeenCalledWith(
      expect.stringContaining("https://termcn.dev/create?preset=b123")
    );
  ...
  it("prints fallback markers when resolving a project preset", async () => {
    ...
    expect(logger.log).toHaveBeenCalledWith(
      "  * Uses preset defaults for values not available as options on termcn/create."
    );
  });

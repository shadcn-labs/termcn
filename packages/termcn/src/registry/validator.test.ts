...
describe("validateRegistryConfig", () => {
  ...
  describe("built-in registries", () => {
    it("should not throw for @termcn since it's now a built-in registry", () => {
      expect(() => {
        validateRegistryConfig("@termcn", {
    ...
    it("should not throw for similar but different registry names", () => {
      expect(() => {
        validateRegistryConfig("@termcn-ui", {
          url: "https://example.com/{name}",
        });
      }).not.toThrow();

      expect(() => {
        validateRegistryConfig("@mytermcn", {
          url: "https://example.com/{name}",
        });
      }).not.toThrow();
    });

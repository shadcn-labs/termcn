...
export class RegistryParseError extends RegistryError {
  ...
  constructor(
    ...
    super(message, {
      ...
      suggestion:
        options.suggestion ??
        `The registry item may be corrupted or have an invalid format. Please make sure it returns a valid JSON object. See ${TERMCN_URL}/schema/registry-item.json.`,
    });
...
export class RegistryInvalidNamespaceError extends RegistryError {
  constructor(public readonly name: string) {
    const message = `Invalid registry namespace: "${name}". Registry names must start with @ (e.g., @termcn, @v0).`;
...
export class RegistriesIndexParseError extends RegistryError {
  ...
  constructor(parseError: unknown) {
    ...
    super(message, {
      ...
      context: { parseError },
      suggestion:
        "The registries index may be corrupted or have invalid registry namespace format. Registry names must start with @ (e.g., @termcn, @example).",
    });

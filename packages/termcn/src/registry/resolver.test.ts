...
describe("resolveRegistryItemsFromRegistries", () => {
  ...
  it("should resolve non-registry items through @termcn registry", () => {
    ...
    expect(result).toEqual([
      "https://termcn.dev/r/styles/{style}/button.json",
      "https://termcn.dev/r/styles/{style}/card.json",
      "https://termcn.dev/r/styles/{style}/dialog.json",
    ]);
    expect(setRegistryHeaders).toHaveBeenCalledWith({});
  });
  ...
  it("should handle mixed registry and non-registry items", () => {
    ...
    // Non-registry items (button, dialog) are resolved through the built-in @termcn registry
    expect(result).toEqual([
      "https://termcn.dev/r/styles/{style}/button.json",
      "https://v0.dev/chat/b/card/json",
      "https://termcn.dev/r/styles/{style}/dialog.json",
...
describe("resolveRegistryTree universal validation", () => {
  it("rejects non-universal dependencies", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(tmpdir(), "termcn-universal-tree-")
    );

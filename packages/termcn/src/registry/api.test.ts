...
const server = setupServer(
  ...
  http.get(`${REGISTRY_URL}/registries.json`, () => {
    return HttpResponse.json([
      {
        name: "@termcn",
        url: "https://termcn.dev/r/styles/{style}/{name}.json",
        description: "The official termcn/ui registry.",
      },
      {
        name: "@example",
...
describe("registry metadata", () => {
  it("throws when the registry index cannot be fetched", async () => {
    ...
    await expect(getTermcnRegistryIndex()).rejects.toThrow();
  });
...
describe("getRegistryItem", () => {
  it("should read and parse a valid local JSON file", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tmpdir(), "termcn-test-"));
  ...
  it("should handle tilde (~) home directory paths", async () => {
    ...
    const tempFile = path.join(homeDir, "termcn-test-tilde.json");
    ...
    try {
      ...
      const tildeePath = "~/termcn-test-tilde.json";
...
describe("getRegistry", () => {
  ...
  it("should throw RegistryParseError with proper context", async () => {
    ...
    try {
      ...
      if (error instanceof RegistryParseError) {
        ...
        expect(error.suggestion).toContain(
          "https://termcn.dev/schema/registry.json"
        );
  ...
  it("should distinguish between URL and registry name", async () => {
    ...
    const registryUrl = "https://termcn.dev/registry.json";

    // Mock for URL
    server.use(
      http.get(registryUrl, () => {
        return HttpResponse.json({
          name: "termcn-from-url",
    ...
    const mockConfig = {
      ...
      registries: {
        "@termcn": {
          url: "https://termcn.dev/{name}.json",
        },
      },
    } as any;
    ...
    expect(urlResult.name).toBe("termcn-from-url");
...
describe("getRegistriesConfig", () => {
  ...
  it("should handle complex registry configurations", async () => {
    ...
    const config = {
      style: "new-york",
      registries: {
        "@termcn": "https://termcn.dev/r/styles/{style}/{name}.json",

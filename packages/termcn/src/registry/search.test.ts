...
describe("searchRegistries", () => {
  it("should fetch and return registries in flat format", async () => {
    ...
    mockGetRegistry.mockImplementation(async (name: string) => {
      if (name === "@termcn" || name === "@termcn/registry") {
        return {
          name: "termcn/ui",
    ...
    const results = await searchRegistries(["@termcn", "@custom"]);
    ...
    expect(results).toEqual({
      items: [
        ...
        },
        {
          ...
          description: "A card component",
          registry: "@termcn",
          addCommandArgument: "@termcn/card",
        },
        {
          name: "header",
          type: "registry:component",
  ...
  it("should apply search filter when query is provided", async () => {
    ...
    const results = await searchRegistries(["@termcn"], { query: "button" });

    expect(results.items).toHaveLength(1);
    expect(results.items[0].name).toBe("button");
    expect(results.items[0].registry).toBe("@termcn");
    expect(results.items[0].addCommandArgument).toBe("@termcn/button");
  ...
  it("should handle mixed registry names and URLs", async () => {
    const registryName = "@termcn";
    const registryUrl = "https://custom.com/registry.json";

    const mockGetRegistry = vi.mocked(getRegistry);

    mockGetRegistry.mockImplementation(async (nameOrUrl: string) => {
      if (nameOrUrl === "@termcn" || nameOrUrl === "@termcn/registry") {
        return {
          name: "termcn/ui",
          homepage: "https://termcn.dev",
          items: [
    ...
    // Should find the button from @termcn
    expect(results.items).toHaveLength(1);
    expect(results.items[0]).toMatchObject({
      name: "button",
...
describe("printSearchResults", () => {
  it("prints type and description inline", async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining('Found 2 items matching "button" in @termcn')
    );
    expect(log).toHaveBeenCalledWith(
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringMatching(
        /- @termcn\/button \(ui\) — A button component\n- @termcn\/card \(ui\)$/
      )
    );
  ...
  it("includes the type filter in the header (normalized for display)", async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Found 1 item of type ui in @termcn")
    );
  ...
  it("prints a warning when no items are found", async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining('No items found matching "missing" in @termcn')
    );
  ...
  it("returns explicitly provided registries unchanged", () => {
    expect(
      resolveSearchRegistries(["@one", "@two"], {
        registries: { "@termcn": "x/{name}.json", "@one": "y/{name}.json" },
      })
    ).toEqual(["@one", "@two"]);
  });

...
describe("buildUrlFromRegistryConfig", () => {
  ...
  it("should handle multiple env vars in params", () => {
    ...
    const url = buildUrlFromRegistryConfig("table", config);
    expect(url).toBe(
      "https://api.com/table?token=abc123&version=v2&key=key456"
    );
  });
...
describe("buildUrlAndHeadersForRegistryItem", () => {
  ...
  it("should preserve explicit termcn framework addresses", () => {
    expect(
      buildUrlAndHeadersForRegistryItem("@termcn/ink/button", {} as any)
    ).toEqual({
      url: "https://termcn.dev/r/ink/button.json",
      headers: {},
    });
    expect(
      buildUrlAndHeadersForRegistryItem("@termcn/opentui/button", {} as any)
    ).toEqual({
      url: "https://termcn.dev/r/opentui/button.json",
      headers: {},
    });

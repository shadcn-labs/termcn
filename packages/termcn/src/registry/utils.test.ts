...
describe("isUrl", () => {
  it("should return true for valid URLs", () => {
    ...
    expect(isUrl("https://termcn.dev/r/styles/new-york/button.json")).toBe(
      true
    );
  });
...
describe("isLocalFile", () => {
  ...
  it("should return false for URLs ending with .json", () => {
    ...
    expect(
      isLocalFile("https://termcn.dev/r/styles/new-york/button.json")
    ).toBe(false);
  });
...
describe("deduplicateFilesByTarget", () => {
  ...
  test("should fallback to concatenation when config is incomplete", async () => {
    ...
    const result = await deduplicateFilesByTarget(
      filesArrays,
      incompleteConfig
    );

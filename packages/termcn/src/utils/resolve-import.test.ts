...
describe("resolve package imports", () => {
  const cwd = getFixturesDir("with-package-imports");
  const config = {
    absoluteBaseUrl: cwd,
    paths: {},
    cwd,
  ...
  it("resolves wildcard imports that preserve extensions", async () => {
  ...
  it("resolves wildcard imports that strip extensions", async () => {
    const result = await resolveImportWithMetadata(
  ...
  it("resolves the root alias for wildcard package imports", async () => {
    expect(await resolveImport("#components", config)).toEqual(
      path.resolve(cwd, "src/components")
    );
  });

  it("resolves exact imports and prefers local conditional targets", async () => {
  ...
  it("ignores package import targets outside the package", async () => {
    expect(resolvePackageImport("#outside/file", cwd)).toBeNull();
  });

  it("falls back to tsconfig paths when package imports do not match", async () => {
    expect(
      ...
      path: "/Users/termcn/Projects/foobar/src/components/ui",
  ...
  it("resolves @/ via tsconfig paths and #/ via package imports in a mixed project", async () => {
...
describe("resolve workspace package exports", () => {
  const root = getFixturesDir("frameworks", "vite-monorepo-imports");
  ...
  it("resolves workspace package wildcard exports for file imports", async () => {
    const result = await resolveImportWithMetadata(
  ...
  it("resolves bare alias roots from workspace package wildcard exports", async () => {
  ...
  it("does not treat workspace package exports as local alias imports", () => {

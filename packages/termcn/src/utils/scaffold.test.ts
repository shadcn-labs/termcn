...
describe("defaultScaffold", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    delete process.env.TERMCN_TEMPLATE_DIR;
    delete process.env.TERMCN_GITHUB_URL;
    mockExit = vi
      .spyOn(process, "exit")
  ...
  it("should clone the repo with sparse checkout", async () => {
    ...
    expect(vi.mocked(execa)).toHaveBeenCalledWith("git", [
      ...
      "--filter=blob:none",
      "--sparse",
      "https://github.com/termcn-ui/ui.git",
      expect.stringContaining("termcn-template-"),
    ]);
  ...
  it("should use local templates when TERMCN_TEMPLATE_DIR is set", async () => {
    process.env.TERMCN_TEMPLATE_DIR = "/local/templates";

...
describe("getProjectComponents", () => {
  ...
  it("should return only components that exist in the registry", async () => {
    ...
    vi.mocked(getTermcnRegistryIndex).mockResolvedValue([
      { name: "button" },
      { name: "card" },
      { name: "dialog" },
    ] as any);
  ...
  it("should throw when the registry index cannot be loaded", async () => {
    ...
    vi.mocked(getTermcnRegistryIndex).mockRejectedValue(
...
describe("get ts config alias prefix", async () => {
  ...
  it("parses JSONC tsconfig files with trailing commas", async () => {
    const cwd = await fsExtra.mkdtemp(
      path.join(tmpdir(), "termcn-jsonc-tsconfig-")
    );

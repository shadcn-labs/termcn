...
import { REGISTRY_URL, TERMCN_URL } from "@/src/registry/constants";
...
export function resolveCreateUrl(
  ...
  const url = new URL(`${TERMCN_URL}/create`);
...
export function resolveInitUrl(
  ...
  return `${TERMCN_URL}/init?${params.toString()}`;
}
...
export async function promptForPreset(options: {
  ...
  const { selectedPreset } = await prompts({
    ...
    choices: [
      ...
      {
        title: "Custom",
        description: `Build your own at ${highlighter.info(`${TERMCN_URL}/create`)}`,
        value: "custom",
      },
    ],
  });
  ...
  const preset =
    DEFAULT_PRESETS[selectedPreset as keyof typeof DEFAULT_PRESETS];
  if (!preset) {
    process.exit(1);
  }
...
function isTermcnInitUrl(url: string) {
  try {
    return new URL(url).pathname === "/init" && url.startsWith(TERMCN_URL);
  } catch {
    return false;
  }
}
